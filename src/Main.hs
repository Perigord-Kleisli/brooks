...
compileOpts argv = case getOpt RequireOrder options argv of
  ...
  where
    header = "Usage: brooks [OPTION...] files..."
  ...
  if settings ^. replMode
    then repl
      ...
      when (settings ^. parse)
        $ forM_
          files
          (either (hPrint stderr) (putStrLn . showTree . unappendAdd) . parser)
      when (settings ^. reduction)
        $ forM_
          files
          (either (hPrint stderr) (putStrLn . betaReduction . unappendAdd)
           . parser)
      forM_
        files
        (either (hPutStrLn stderr) (print @Int . ($ 1)) . (`runLambda` (+ 1)))
